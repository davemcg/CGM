...
gzcat $gtf | \
	awk '$3 == "gene" {print $0}' | \
	grep "protein_coding" | \
	bedtools sort -i - > $prefix.gene.gtf

# count peaks overlapping each gtf gene
bedtools intersect -a $prefix.gene.gtf -b $peaks -F 0.5 -c > $prefix.gene_peak_overlaps.data
...
# bedtools closest -a <( bedtools intersect -a $peaks -b /tmp/$prefix.gene.gtf -v ) \ # (removing peaks that overlap genes)
bedtools closest -a $peaks -b $prefix.gene.gtf -k 5  -D "a" -io | \
