...
library(gread)
# gene=commandArgs(trailingOnly = T)[1]
# gene='OR4F5'
...
make_matrix <- function(gtf,gene,target_peaks){
    ...
    peaks_range <- GRanges(seqnames=target_peaks$p_chr, ranges = IRanges(start=target_peaks$p_start,end = target_peaks$p_end),peaks=target_peaks$peak)    
...
make_matrix(gtf,gene,target_peaks)
a=Sys.time()
all_plus_genes <- lapply(all_genes,function(x) make_matrix(gtf,x,target_peaks))
b=Sys.time()
#current runtime for 
